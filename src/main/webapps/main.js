...
function displayTasks(tasks) {
    ...
    tasks.forEach(task => {
        if(task.isCompleted) {
            ...
            `<tr id="task-id-${task.id}" class="row">
            ...
            `<tr id="task-id-${task.id}" class="row">
                ...
                <td class="col-2 task-due-date">${task.dueDate.map(data => (data.toString().length < 2) ? `0${data}` : data.toString()).join("-")}</td>
                <td class="text-center col-1">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-check-lg icon" viewBox="0 0 16 16" data-bs-toggle="modal" data-bs-target="#completeTaskModal" data-bs-task-id="${task.id}">
